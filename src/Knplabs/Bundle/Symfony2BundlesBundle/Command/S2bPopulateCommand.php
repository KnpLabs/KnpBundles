...
class S2bPopulateCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $repos = array();
        foreach ($em->getRepository('Knplabs\Bundle\Symfony2BundlesBundle\Entity\Repo')->findAll() as $repo) {
            $repos[strtolower($repo->getFullName())] = $repo;
        }
        $users = array();
        foreach ($em->getRepository('Knplabs\Bundle\Symfony2BundlesBundle\Entity\User')->findAll() as $user) {
